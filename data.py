...
class DiscriData(data.Dataset):
    """DiscriData."""
    def __init__(self, config, num_X0s=100, num_traces=10, num_t=100, data_file=None, shuffle=True):
...
def get_dataloader(config, num_X0s_train, num_X0s_val, batch_size=16, data_file=[None, None]):
    train_loader = torch.utils.data.DataLoader(
        DiscriData(config, num_X0s_train, data_file=data_file[0]), batch_size=batch_size, shuffle=True,
        num_workers=20, pin_memory=True)

    val_loader = torch.utils.data.DataLoader(
        DiscriData(config, num_X0s_val, data_file=data_file[1]), batch_size=batch_size, shuffle=True,
        num_workers=20, pin_memory=True)
